...
| projectDefaults.projectApplication.syncPolicy | object | `{"automated":{"enabled":true,"prune":true,"selfHeal":true},"syncOptions":["Prune=confirm","Delete=confirm"]}` | Optional. Sync policy for the `<project>-project` Applications: auto-sync + self-heal, with prune/cascade-delete gated behind manual UI confirmation. Declares `enabled` so a UI auto-sync toggle surfaces as drift (ArgoCD 3.1+). |
