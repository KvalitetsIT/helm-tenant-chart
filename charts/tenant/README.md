...
| projectDefaults.projectApplication.syncPolicy | object | `{"automated":{"prune":true,"selfHeal":true},"syncOptions":["Prune=confirm","Delete=confirm"]}` | Optional. Sync policy applied to all project Applications. Prune and cascade-delete are gated behind manual confirmation (`Prune=confirm`, `Delete=confirm`): the `<project>-project` Applications auto-sync and self-heal, but structure is never deleted without an explicit click in the UI. Needs ArgoCD 2.13+. |
